...
public class SvgKit {
     ...
     * T x,y
     */
    public static Path parser(String svgPath) {
        List<Integer> cmdPositions = findCommand(svgPath);
        ...
        for (int i = 0; i < cmdPositions.size(); i++) {
            ...
            switch (svgPath.charAt(index)) {
                case 'm':
                case 'M': {
                    String ps[] = findPoints(i, svgPath, cmdPositions);
    ...
    private static List<Integer> findCommand(String svgPath) {
        int svgPathLength = svgPath.length();
        List<Integer> cmdPositions = new ArrayList<>();
        int mIndex = 0;
    ...
    private static String[] findPoints(int cmdIndexInPosition, String svgPath, List<Integer> cmdPositions) {
